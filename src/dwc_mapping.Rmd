...
literature_references %<>% separate(
  input_source, 
  into = c(paste("reference", c(1,2,3), sep = "_")), # New column name
  sep = " \\| ",
  extra = "drop" # 4th value or more will be dropped
)
```
...
literature_references %<>% 
  gather(key, value, starts_with("reference_"), na.rm = TRUE) %>% 
...
Use the `input_source` field as is. Its content should be ` | ` separated for more than one reference.
...
Only keep records for which the `input_realm` is not empty:
