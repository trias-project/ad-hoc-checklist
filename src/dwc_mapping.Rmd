...
All duplicates differ in their distribution information, i.e. `location`, `date_first_observation` and `date_last_observation`. For the mapping of the taxon core, the literature reference extension, the species profile extension and the description extension, we can remove these duplicated taxa. Before doing this, we generate the data frame `distribution` here (containingin the duplicated rows) to continue the mapping in (reference to distribution section):
...
colnames(distribution) <- paste0("raw_", colnames(distribution))
```
...
Separate `raw_source` by using `|` as a separator:
...
```{r}
native_range %<>% filter(duplicated(native_range $ raw_scientific_name)==FALSE)
```
...
```{r}
native_range %<>% separate(raw_native_range, into = paste0("range", c(1:3)), sep = "\\| ", remove = F)
```

Gather pathway information in `value`:
...
```{r}
native_range %<>% mutate(value = str_replace_all(value, "\\?", ""))
```

Capitalize first letters:

```{r}
native_range %<>% mutate(value = str_to_title(value)) 
```
...
All terms are mapped to the CBD standard. 

Separate data based on pipe:

```{r}
pathway_desc %<>% separate(raw_introduction_pathway, into = c(paste("pathway", c(1,2), sep = "_")), sep = "\\|")
```
